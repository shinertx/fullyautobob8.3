...
class SimLab:
    ...
    def _stats(self, returns: pd.Series) -> dict:
        ...
        return {
            ...
            "sortino": float(returns.mean()/down_std) if down_std and down_std>0 else 0.0,
            "sharpe": float(returns.mean()/returns.std()) if returns.std() and returns.std()>0 else 0.0,
            "mdd": float(dd.min()),
