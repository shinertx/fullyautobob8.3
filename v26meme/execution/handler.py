...
class ExecutionHandler:
    ...
    def _min_cost_ok(self, ex_sym: str, usd_value: float) -> bool:
        try:
            ...
            return True if (min_cost is None) else (usd_value >= float(min_cost))
        except Exception:
            return True

    def _amt_to_precision(self, ex_sym: str, amount: float) -> float:
        try:
    ...
    def reconcile(self, target_weights: dict, active_alphas: list):
        ...
        for sym in set(list(symbol_target.keys()) + list(positions.keys())):
            ...
            price_cache[sym] = px
            if sym in positions:
                amt = float(positions[sym].get('amount', 0.0))
                equity_mark += amt * px
        ...
        if self.cfg['mode'] == 'paper':
            for sym, w in (symbol_target or {}).items():
                ...
                if not self._min_cost_ok(ex_sym, abs(delta_usd)):
                    logger.info(f"{ex_sym} trade ${abs(delta_usd):.2f} below min notional; skipping")
                    continue

                side = 'buy' if delta_usd > 0 else 'sell'
                fill_px = px * (1 + self.slippage) if side == 'buy' else px * (1 - self.slippage)
                qty = abs(delta_usd) / fill_px
                qty = self._amt_to_precision(ex_sym, qty)
                if qty <= 0: continue
        ...
        if self.cfg['mode'] == 'live':
            logger.warning("[LIVE MODE] Not recommended until you verify paper stats.")
            for sym, w in (symbol_target or {}).items():
                ex_sym = self._ex_symbol(sym)
                if not ex_sym: continue
                px = price_cache.get(sym, 0.0)
                ...
                if not self._min_cost_ok(ex_sym, abs(delta_usd)):
                    ...
                    continue
                side = 'buy' if delta_usd > 0 else 'sell'
                qty = abs(delta_usd) / px
                qty = self._amt_to_precision(ex_sym, qty)
                if qty <= 0: continue
                try:
                    order = self.exchange.create_order(symbol=ex_sym, type='market', side=side, amount=qty)
                    logger.info(f"LIVE {side.upper()} {ex_sym} qty={qty} -> {order.get('id','')}")
                except Exception as e:
                    logger.error(f"Order failed for {ex_sym}: {e}")
            return
